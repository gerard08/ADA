...
CString NombreAlumno1 = "Gerard";
CString ApellidosAlumno1 = "Martínez Espelleta";
CString NIUAlumno1 = "1531236"; // NIU alumno1
...
bool EjecutarLineaDeComandos(int nArgs, CString args[])
{
	...
	if (args[1].CompareNoCase("Prim") == 0) {
		...
		try {
			...
			double t0 = Clock();
			DijkstraQueue(graph, &graph.m_Vertices.front());
			double t1 = Clock();
			cout << endl << "TIEMPO DE EJECUCION: " << fixed << t1 - t0 << endl;
			graph.SaveDistances(args[3]);
		}
